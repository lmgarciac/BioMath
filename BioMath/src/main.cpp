...
static const int   kFramesPerBuffer = 2048; // chico = baja latencia. duración del buffer: 2048 / 48000 = 42.6 ms

// Porque si uso 1024 frames se siente "glitchy":
// Porque con 1024 frames a 48 kHz cada buffer dura 21 ms, así que el sistema tiene muy poco margen para generar y entregar 
...
// de unos pocos milisegundos (5–10 ms, algo normal) provoca un underrun audible como glitch. Con 2048 frames, cada buffer dura 43 ms, 
...
// Aunque tenga más latencia.

static const int   kNumBuffers = 2; // mientras uno suena, el otro se rellena

static WAVEHDR g_waveHdr[kNumBuffers]{}; //estructura WinMM para saber qué reproducir.
...
// Función para llenar buffer. Esto genera un seno y le agrega un poco de cuerpo con armónicos suaves.

static void FillAudioBuffer(short* outInterleaved, int frames)
{
	// outInterleaved: puntero al buffer de salida estéreo intercalado
	...
	// Paso de fase del oscilador; phaseStep = radianes que avanzo por sample p ej: 440 Hz a 48 kHz → ~0.0576 rad/sample
	...
	for (int i = 0; i < frames; ++i)
	{
		...
		// un par de armónicos suaves (opcional para suavizar sonido)
		s += 0.25 * sin(g_phase * 2.0);
		s += 0.12 * sin(g_phase * 3.0);

		// normalización suave
		// Como sume armónicos, la amplitud puede pasarse de 1. evita clipping
		s *= 0.6;

		// Conversión a int16 (PCM). El DAC espera int16
		// Con clamp defensivo
		int v = (int)(s * 32767.0);
		if (v > 32767) v = 32767;
		if (v < -32768) v = -32768;
		...
		if (g_phase >= twoPi) g_phase -= twoPi; //la fase siempre en /0, 2π)
	}
}

// Callback de waveOut
// Cuando un buffer terminó de reproducirse, el sistema manda WOM_DONE. Lo rellenamos y lo reenviamos.
...
static bool InitAudio()
{
	...
	wfx.nBlockAlign = (wfx.nChannels * wfx.wBitsPerSample) / 8; // cuántos bytes ocupa un frame
	...
	MMRESULT r = waveOutOpen(
		...
		WAVE_MAPPER,
		&wfx,
		(DWORD_PTR)WaveOutCallback, // puntero a función callback.
		0,
		CALLBACK_FUNCTION
	);
	...
	{
		FillAudioBuffer(g_audioData[i], kFramesPerBuffer); //llena el buffer i con samples int16 estéreo interleaved
		...
		//Encolar el buffer para reproducción
